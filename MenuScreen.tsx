...
const MenuScreen: React.FC<{ route: any; navigation: any }> = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    if (dishes.length > 0) {
      ...
      setAveragePrice(totalPrice / dishes.length);
    } else {
      setAveragePrice(0);
    }
  }, [dishes]);
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity onPress={() => navigation.navigate('FilterSearchScreen')}>
  ...
  <TouchableOpacity onPress={() => navigation.navigate('DeleteScreen', { dishes, setDishes })} >
      <Image style={styles.deleteButton} source={require('./assets/recycle-bin.png')} />
       
      </TouchableOpacity>

      <Text style={styles.chef1}>Chef</Text>
